...
namespace DAL.Framework
{
    public abstract class BaseData<T> where T : IBaseEntity
    {
        ...
        public abstract void Delete(DBDataContext db, long id, Binary version);
        public abstract void Delete(DBDataContext db, string id, Binary version);
        ...
        protected bool UpdateKey_Value(DBDataContext db, string SourceEntity, string OldValue, string NewValue)
        {
            return (db.UpdatePrimaryKey(SourceEntity, OldValue, NewValue) == 1);
        }
        #region Common Functions
        protected static bool IsDuplicate(DBDataContext db, string tableName, string fieldName,
